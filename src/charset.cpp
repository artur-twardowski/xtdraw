...
uint32_t GetBoxDrawingCharacter(const box_drawing_spec_t &s) {
    ...
    if (s.north == s.south && s.east == s.west && s.north != line_weight_t::NONE && s.east == line_weight_t::NONE) {
        ...
        size_t index = (static_cast<size_t>(s.north) << 6) | (static_cast<size_t>(s.south) << 4) |
...
static constexpr uint32_t BLOCKS_2x2[] = {0x0020, 0x2598, 0x259d, 0x2580, 0x2596, 0x258c, 0x259e, 0x259b,
